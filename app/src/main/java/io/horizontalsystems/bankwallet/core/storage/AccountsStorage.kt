                ...
                .mapNotNull { record: AccountRecord ->
                    try {
                        val accountType = when (record.type) {
                            ...
                            OXYRA_WATCH_ACCOUNT -> AccountType.OxyraWatchAccount.fromSerialized(record.key!!.value)
                            OXYRA_WATCH_ACCOUNT -> AccountType.OxyraWatchAccount.fromSerialized(record.key!!.value)
                            else -> null
                        }
                        Account(
    ...
    private fun getAccountRecord(account: Account): AccountRecord {
        ...
        when (account.type) {
            ...
            is AccountType.OxyraWatchAccount -> {
                val oxyraAccount = account.type as AccountType.OxyraWatchAccount
                key = SecretString(oxyraAccount.serialized)
                accountType = OXYRA_WATCH_ACCOUNT
            }
        }
